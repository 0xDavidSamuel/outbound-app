...
export default function NavBar() {
    ...
    (async () => {
      ...
      try {
        const nRes = await fetch(
          `${SUPABASE_URL}/rest/v1/notifications?user_id=eq.${session.user.id}&select=*&order=created_at.desc&limit=30`,
          { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${session.access_token}` } }
        );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
          `${SUPABASE_URL}/rest/v1/notifications?user_id=eq.${userId}&select=*&order=created_at.desc&limit=30`,
                  ...
                  notifications.slice(0, 20).map(n => (
                    <div key={n.id} className={`nav-notif${!n.read ? ' unread' : ''}`}>
                      ...
                      <div className="nav-notif-body">
                        ...
                        <div className="nav-notif-time">{timeAgo(n.created_at)}</div>
                      </div>
