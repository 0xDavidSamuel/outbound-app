...
interface RateModalProps {
  city: City;
  onClose: () => void;
  onSave: (slug: string, ratings: any) => void;
}
...
function RateModal({ city, onClose, onSave }: RateModalProps) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 28, width: '100%', maxWidth: 400, fontFamily: 'DM Sans, sans-serif',
      }}>
              ...
              {[1,2,3,4,5].map(star => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 18, color: star <= (ratings as any)[key] ? '#e8ff47' : '#222',
        ...
        <textarea
          ...
          style={{
            ...
            padding: '10px 14px', color: '#fff', fontFamily: 'DM Sans, sans-serif', fontSize: 12,
            resize: 'none', height: 72, outline: 'none', marginBottom: 16,
          }}
        />

        <button
          onClick={() => onSave(city.slug, { ...ratings, note, city_name: city.name })}
          style={{
...
export default function CitiesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (session) {
        ...
        const { data: ratings } = await supabase.from('city_ratings').select('*').eq('user_id', session.user.id);
        const ratingMap: Record<string, any> = {};
        (ratings || []).forEach((r: any) => { ratingMap[r.city_slug] = r; });
  ...
  const saveRating = async (slug: string, ratings: any) => {
    if (!user) return;
    await supabase.from('city_ratings').upsert({
      user_id: user.id,
      city_slug: slug,
      city_name: ratings.city_name,
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-rate.rated { background: rgba(232,255,71,0.12); color: #e8ff47; }
        ...
        .section-divider-label {
          ...
          letter-spacing: 0.4em; color: #222; text-transform: uppercase; white-space: nowrap;
        }
        ...
        .loading-state {
          ...
          font-family: 'DM Mono', monospace; font-size: 11px; color: #333; letter-spacing: 0.2em;
        }
        .loading-dot {
        ...
        @media (max-width: 600px) {
          .cities-page { padding: 64px 16px 140px; }
          .cities-grid { grid-template-columns: 1fr 1fr; }
          .city-list-scores { display: none; }
        }
      `}</style>
      ...
      <div className="cities-page">
        ...
        <div className="section-divider">
          <span className="section-divider-label">Travel Tools</span>
        ...
        {!loading && view === 'grid' && (
          <div className="cities-grid">
            {filtered.map((city, i) => {
              const filterKey = FILTERS[activeFilter].key;
              const relevantScore = filterKey === 'overall' ? city.overall : city.scores[filterKey];
                ...
                <div key={city.slug} className="city-card">
                  <div className="city-image">
                    {city.image
                      ...
                      : <div className="city-image-placeholder">{city.name.slice(0,2).toUpperCase()}</div>
                    }
                    <div className="city-rank">{i + 1}</div>
                    {city.overall && (
                      <div className="city-overall">★ {(city.overall / 10).toFixed(1)}</div>
                    )}
                  </div>
                  <div className="city-body">
                      ...
                      {['Internet Access', 'Cost of Living', 'Safety'].map(key => {
                        ...
                        return (
                          <div key={key} className="city-score-row">
                            ...
                            <div className="city-score-bar">
                              <div className="city-score-fill" style={{ width: `${val * 10}%`, background: scoreColor(val) }} />
                            </div>
                    ...
                    <div className="city-actions">
                      <button
                        className={`btn-rate${hasRated ? ' rated' : ''}`}
                        onClick={() => setRatingModal(city)}
                      >
