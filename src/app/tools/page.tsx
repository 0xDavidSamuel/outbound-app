...
const CURRENCIES = [
  ...
  'THB','IDR','MYR','PHP','AED','SAR','COP','PEN','CLP','NGN',
  'KES','GHS','EGP','MAD','TWD','VND','PKR','BDT','ILS','PLN',
];
...
export default function ToolsPage() {
  ...
  const translate = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(inputText)}&langpair=${langPair}`);
      const data = await res.json();
      ...
      setTransError('Translation service unavailable.');
    }
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .copy-btn { background: none; border: 1px solid #1a1a1a; color: #444; border-radius: 6px; padding: 5px 12px; font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.15em; text-transform: uppercase; cursor: pointer; transition: all 0.15s; float: right; margin-bottom: 8px; }
        ...
        .tool-link-name { font-size: 13px; font-weight: 500; color: '#fff'; }
      ...
      <div className="tools-page">
        ...
        <div className="tab-row">
          <button className={`tab-btn${activeTab === 'currency' ? ' active' : ''}`} onClick={() => setActiveTab('currency')}>
            💱 Currency
          </button>
          ...
          <div className="tool-card">
            ...
            {convertedAmount !== null && (
              <div className="result-box">
                ...
                <div className="result-rate">
                  ...
                  {rateUpdated && ` · Updated ${rateUpdated}`}
                </div>
              </div>
            )}
            ...
            <div className="quick-amounts">
              ...
              {[100, 500, 1000, 5000, 10000].map(n => (
                <button key={n} className="quick-btn" onClick={() => { setAmount(String(n)); setConvertedAmount(null); }}>
                  {n.toLocaleString()}
                </button>
              ))}
            </div>
        ...
        {activeTab === 'translate' && (
          <div className="tool-card">
            ...
            {outputText && (
              <>
                <div className="tool-label" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button className="copy-btn" style={{ float: 'none' }} onClick={() => navigator.clipboard.writeText(outputText)}>Copy</button>
                </div>
            ...
            <div style={{ marginTop: 16, fontFamily: 'DM Mono, monospace', fontSize: 9, color: '#1e1e1e', letterSpacing: '0.15em' }}>
              Powered by MyMemory · 5000 chars/day free
            </div>
          </div>
        )}
          ...
          <div className="tools-grid">
            {[
              ...
              <a key={t.name} className="tool-link" href={t.url} target="_blank" rel="noopener noreferrer">
                ...
                <div className="tool-link-name" style={{ color: '#fff' }}>{t.name}</div>
