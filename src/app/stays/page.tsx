...
function ListStayModal({ onClose, onSave, userId }: { onClose: () => void; onSave: (stay: any) => void; userId: string }) {
  const [form, setForm] = useState({
    ...
    type: 'apartment', bedrooms: 1, bathrooms: 1, max_guests: 2,
    wifi_speed: '', amenities: [] as string[], images: [''],
    is_nomad_friendly: true,
  });
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, overflowY: 'auto' }}>
      <div style={{ background: '#0d0d0d', border: '1px solid #1a1a1a', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, fontFamily: 'DM Sans, sans-serif' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 8, letterSpacing: '0.4em', color: '#333', textTransform: 'uppercase', marginBottom: 4 }}>
              Step {step} of 2
            </div>
            <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, color: '#e8ff47', lineHeight: 1 }}>
              {step === 1 ? 'List your stay.' : 'Pricing & amenities.'}
            </div>
          </div>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <input placeholder="Title (e.g. Sunny studio near beach)" value={form.title} onChange={e => set('title', e.target.value)}
              style={inputStyle} />
            <textarea placeholder="Describe your place — who it's for, vibe, neighbourhood..." value={form.description} onChange={e => set('description', e.target.value)}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {['apartment', 'house', 'room', 'coliving', 'villa', 'studio'].map(t => (
                  <button key={t} onClick={() => set('type', t)}
                    style={{ ...pillStyle, ...(form.type === t ? pillActiveStyle : {}) }}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              {form.images.map((img, i) => (
                <input key={i} placeholder={`Image ${i + 1} URL`} value={img}
                  onChange={e => { const imgs = [...form.images]; imgs[i] = e.target.value; set('images', imgs); }}
                  style={{ ...inputStyle, marginBottom: 6 }} />
              ))}
              <button onClick={() => set('images', [...form.images, ''])}
                style={{ fontFamily: 'DM Mono, monospace', fontSize: 9, color: '#444', background: 'none', border: 'none', cursor: 'pointer', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
                + Add image
              </button>
            </div>
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {AMENITIES_LIST.map(a => (
                  <button key={a} onClick={() => toggleAmenity(a)}
                    style={{ ...pillStyle, ...(form.amenities.includes(a) ? pillActiveStyle : {}) }}>
                    {a}
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: '#111', border: '1px solid #1a1a1a', borderRadius: 8 }}>
              ...
              <button onClick={() => set('is_nomad_friendly', !form.is_nomad_friendly)}
                style={{ width: 40, height: 22, borderRadius: 11, background: form.is_nomad_friendly ? '#e8ff47' : '#1a1a1a', border: 'none', cursor: 'pointer', transition: 'background 0.2s', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => onSave({
                ...form,
                price_per_month: parseInt(form.price_per_month) || 0,
                price_per_week: parseInt(form.price_per_week) || null,
                price_per_night: parseInt(form.price_per_night) || null,
                wifi_speed: parseInt(form.wifi_speed) || null,
...
const inputStyle: React.CSSProperties = {
  width: '100%', background: '#111', border: '1px solid #1a1a1a', borderRadius: 8,
  padding: '10px 14px', color: '#fff', fontFamily: 'DM Sans, sans-serif', fontSize: 13,
  outline: 'none',
};
...
const pillActiveStyle: React.CSSProperties = {
  background: '#e8ff47', color: '#080808', border: '1px solid #e8ff47',
};
const primaryBtnStyle: React.CSSProperties = {
  width: '100%', background: '#e8ff47', color: '#080808', border: 'none',
  borderRadius: 8, padding: '13px', fontFamily: 'DM Mono, monospace',
  fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase',
  cursor: 'pointer', fontWeight: 500,
};
const countBtnStyle: React.CSSProperties = {
  width: 28, height: 28, borderRadius: 6, background: '#111',
  border: '1px solid #1a1a1a', color: '#fff', cursor: 'pointer',
  fontFamily: 'DM Mono, monospace', fontSize: 14, display: 'flex',
...
export default function StaysPage() {
  ...
  const saveStay = async (stay: any) => {
    const { data, error } = await supabase.from('stays').insert(stay).select('*, host:profiles(username, avatar_url)').single();
    if (!error && data) {
      setStays(prev => [data, ...prev]);
      setShowModal(false);
    }
  };
    ...
    <>
      <style suppressHydrationWarning>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-list { font-family: 'DM Mono', monospace; font-size: 9px; letter-spacing: 0.2em; text-transform: uppercase; padding: 9px 18px; border-radius: 8px; background: #e8ff47; color: #080808; border: none; cursor: pointer; font-weight: 500; white-space: nowrap; transition: opacity 0.2s; }
        ...
        .stay-price { font-family: 'Bebas Neue', sans-serif; font-size: 28px; color: '#fff'; line-height: 1; }
        .stay-price span { font-family: 'DM Mono', monospace; font-size: 9px; color: '#444'; letter-spacing: 0.1em; }
        ...
        .wifi-badge { display: flex; align-items: center; gap: 4px; font-family: 'DM Mono', monospace; font-size: 9px; color: #444; }
        ...
        .detail-price-amount { font-family: 'Bebas Neue', sans-serif; font-size: 28px; color: '#fff'; line-height: 1; }
        ...
        @media (max-width: 600px) {
          .stays-page { padding: 64px 16px 140px; }
          .stays-grid { grid-template-columns: 1fr; }
        }
      `}</style>

      {showModal && user && (
        <ListStayModal onClose={() => setShowModal(false)} onSave={saveStay} userId={user.id} />
      )}
        ...
        <div className="detail-overlay" onClick={() => setSelectedStay(null)}>
          <div className="detail-modal" onClick={e => e.stopPropagation()}>
            <div className="detail-images">
              {selectedStay.images?.[0]
                ? <img src={selectedStay.images[0]} alt={selectedStay.title} />
            ...
            <div className="detail-body">
              ...
              <div className="detail-prices">
                {selectedStay.price_per_month && (
                  <div className="detail-price-item">
                    <div className="detail-price-amount" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, color: '#fff' }}>${selectedStay.price_per_month.toLocaleString()}</div>
                    <div className="detail-price-period">/ month</div>
                  </div>
                )}
                {selectedStay.price_per_week && (
                  <div className="detail-price-item">
                    <div className="detail-price-amount" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, color: '#fff' }}>${selectedStay.price_per_week.toLocaleString()}</div>
                    <div className="detail-price-period">/ week</div>
                  </div>
                )}
                {selectedStay.price_per_night && (
                  <div className="detail-price-item">
                    <div className="detail-price-amount" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 28, color: '#fff' }}>${selectedStay.price_per_night.toLocaleString()}</div>
                    <div className="detail-price-period">/ night</div>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
                {[
                  ['🛏', `${selectedStay.bedrooms} bed${selectedStay.bedrooms > 1 ? 's' : ''}`],
                  ['🚿', `${selectedStay.bathrooms} bath${selectedStay.bathrooms > 1 ? 's' : ''}`],
                  ['👤', `${selectedStay.max_guests} guests`],
                  ...(selectedStay.wifi_speed ? [['📶', `${selectedStay.wifi_speed} Mbps`]] : []),
                ].map(([icon, label]) => (
              ...
              {selectedStay.amenities?.length > 0 && (
                <div className="detail-amenities-grid">
                  {selectedStay.amenities.map(a => (
                    <span key={a} className="detail-amenity" style={{ color: '#444' }}>{a}</span>
                  ))}
                </div>
              )}
              ...
              {selectedStay.host && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 0', borderTop: '1px solid #1a1a1a', marginBottom: 16 }}>
                  {selectedStay.host.avatar_url
                    ? <img src={selectedStay.host.avatar_url} style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '1px solid #222' }} alt="" />
                    : <div style={{ width: 32, height: 32, borderRadius: '50%', background: '#111', border: '1px solid #222', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#444', fontSize: 12 }}>?</div>
                  }
                  <div>
      ...
      <div className="stays-page">
        ...
        <div className="stays-controls">
          ...
          <div className="filter-row">
            ...
            {user && <button className="btn-list" onClick={() => setShowModal(true)}>+ List your stay</button>}
          </div>
        ...
        {!loading && filtered.length === 0 && (
          <div className="empty-state">
            ...
            {user && <button className="empty-cta" onClick={() => setShowModal(true)}>Be the first to list →</button>}
          </div>
        )}
              ...
              <div key={stay.id} className={`stay-card${stay.featured ? ' featured' : ''}`} onClick={() => setSelectedStay(stay)}>
                <div className="stay-image">
                  {stay.images?.[0]
                    ...
                    : <div className="stay-image-placeholder">{stay.type === 'villa' ? '🏖' : stay.type === 'coliving' ? '🏢' : stay.type === 'house' ? '🏡' : '🏠'}</div>
                  }
                ...
                <div className="stay-body">
                  ...
                  {stay.amenities?.length > 0 && (
                    <div className="stay-amenities">
                      {stay.amenities.slice(0, 4).map(a => <span key={a} className="stay-amenity">{a}</span>)}
                      {stay.amenities.length > 4 && <span className="stay-amenity">+{stay.amenities.length - 4}</span>}
                    </div>
                  )}
                  <div className="stay-footer">
                    <div>
                      <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 26, color: '#fff', lineHeight: 1 }}>
                        ${stay.price_per_month.toLocaleString()}
                      </div>
                    ...
                    {stay.host && (
                      <div className="stay-host">
                        {stay.host.avatar_url
                          ? <img className="stay-host-avatar" src={stay.host.avatar_url} alt="" />
                          : <div className="stay-host-avatar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#444' }}>?</div>
                        }
